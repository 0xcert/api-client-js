import { AssetSetOperatorOrder, DappValueApproveOrder, Gateway, OrderKind } from '@0xcert/ethereum-gateway';
import BigNumber from 'bignumber.js';
import { URLSearchParams } from 'url';
import { Client } from '../client';
import { ClientError } from '../helpers/client-error';
import clientFetch from '../helpers/client-fetch';
import { AssetApproveData, ClientErrorCode, DappValueApproveData, GetApprovalsOptions, Priority } from '../types';
...
export class ApprovalsController {
   ...
   */
  public async createDappValueApproval(approveData: DappValueApproveData, priority: Priority) {
    if (!this.context.authentication) {
      throw new ClientError(ClientErrorCode.CLIENT_NOT_CONNECTION);
    }
    ...
    const date = Date.now();
    const feeValue = new BigNumber(this.context.payment.dappValueApproveCost);
    ...
    const approveOrder = {
      ...
      approver: this.context.provider.accountId,
      spender: approveData.spender,
      value: new BigNumber(approveData.value).multipliedBy(multiplier).toFixed(0),
      seed: date,
      expiration: Date.now() + 172800000, // 2 days
      feeValue: feeValue.toFixed(0),
      feeRecipient: this.context.payment.receiverAddress,
    };
    const claim = await gateway.sign(approveOrder as DappValueApproveOrder);
   ...
   * @param priority Priority for this deploy to perform.
   */
  public async createAssetApproval(approveData: AssetApproveData, priority: Priority) {
    if (!this.context.authentication) {
      throw new ClientError(ClientErrorCode.CLIENT_NOT_CONNECTION);
    }
